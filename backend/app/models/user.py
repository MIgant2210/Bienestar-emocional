...
class User(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'avatar_name': (self.avatar_name or (self.avatars[0].name if hasattr(self, 'avatars') and self.avatars else None) or 'Mi Avatar'),
            'avatar_config': (self.avatar_config or (self.avatars[0].config if hasattr(self, 'avatars') and self.avatars else None)),
